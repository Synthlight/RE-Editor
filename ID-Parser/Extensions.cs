...
public static class Extensions {
    ...
    public static Dictionary<Global.LangIndex, Dictionary<TOut, string>> ConvertToByEnumName<TIn, TOut>(this Dictionary<Global.LangIndex, Dictionary<TIn, string>> inDict) where TIn : struct, Enum
                                                                                                                                                                           where TOut : struct, Enum {
        var outDict = new Dictionary<Global.LangIndex, Dictionary<TOut, string>>(inDict.Count);
        foreach (var (lang, innerDict) in inDict) {
            outDict[lang] = new(innerDict.Count);
            foreach (var (key, value) in innerDict) {
                var outKey = Enum.Parse<TOut>(Enum.GetName(key)!);
                outDict[lang][outKey] = value;
            }
        }
        return outDict;
