using System.Numerics;

namespace RE_Editor.Common.PakModels.Encryption;

public static class PakCipher {
    private static readonly BigInteger MODULUS = new([
        
    ]);

    private static readonly BigInteger EXPONENT = new([
        0x01, 0x00, 0x01, 0x00
    ]);

    private static byte[] DecryptKey(byte[] encryptedKeyBytes) {
        // Resize it (this will add 0 to the end of array) to avoid negative values in BigInteger (same for modulus).
        Array.Resize(ref encryptedKeyBytes, 129);

        var encryptedKey = new BigInteger(encryptedKeyBytes);
        var result       = BigInteger.ModPow(encryptedKey, EXPONENT, MODULUS);

        return result.ToByteArray();
    }

    public static byte[] DecryptData(byte[] buffer, byte[] encryptedKey) {
        var lpKey = DecryptKey(encryptedKey);

        if (lpKey.Length > 0) {
            for (var i = 0; i < buffer.Length; i++) {
                buffer[i] ^= (byte) (i + lpKey[i % 32] * lpKey[i % 29]);
            }
        }

        return buffer;
    }
}